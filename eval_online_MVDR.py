...
def lstm_conv_online(corr_mix): 
    corr_mix = corr_mix.permute(0, 2, 1, 3, 4)
    alpha = 0.995
    T = corr_mix.shape[1]
    corr = corr_mix
    for i in range(1, T):
        corr[:, i] = alpha * corr[:, i-1] + (1 - alpha) * corr_mix[:, i]
    corr = corr.permute(0, 2, 1, 3, 4)
    return corr 
...
def online_mvdr_lstm(mix_spec, est_speech ,est_noise, NUM_MIC=5):
    ...
    corr_speech = lstm_conv_online(corr_speech)
    corr_noise = lstm_conv_online(corr_noise)
